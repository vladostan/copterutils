...
logger.write("Images num: {}\n".format(np.size(images)))
logger.write("Labels num: {}\n\n".format(np.size(labels)))
...
pandas.DataFrame(history.history).to_csv("history/{}.csv".format(loggername))

# In[67]:

model.save_weights('weights/{}.hdf5'.format(loggername))

# In[43]:

score = model.evaluate(x_test, y_test_flat, batch_size = 1, verbose=1)
logger.write('Test score:\n', score[0])
